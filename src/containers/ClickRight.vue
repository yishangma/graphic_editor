...
import XIcon from '../global/Icon/index'
    ...
    handleContextMenuList() {
      ...
      this.toolList.forEach(item => {
        if (item.enableTool && item.enable && item.contextmenu && item.contextmenu.enable) {
          if (Object.prototype.hasOwnProperty.call(!item.contextmenu, 'target') || (Object.prototype.hasOwnProperty.call(item.contextmenu, 'target') && (item.contextmenu.target.length && this.options && this.options.type && item.contextmenu.target.includes(this.options.type)))) {
